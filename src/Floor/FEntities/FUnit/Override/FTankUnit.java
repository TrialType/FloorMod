...
public class FTankUnit extends TankUnit implements FUnitUpGrade {
    ...
    public void update(){
        ...
        if (this.mineTile != null) {
            ...
            if (core != null && item != null && !this.acceptsItem(item) && this.within(core, 220.0F) && !this.offloadImmediately()) {
                total = core.acceptStack(this.item(), this.stack().amount, this);
                if (total > 0) {
                    Call.transferItemTo(this, this.item(), total, this.mineTile.worldx() + Mathf.range(4.0F), this.mineTile.worldy() + Mathf.range(4.0F), core);
                    this.clearItem();
                }
            }
        ...
        int dy;
        if ((this.walked || Vars.net.client() && this.deltaLen() >= 0.01F) && !Vars.headless && !this.inFogTo(Vars.player.team())) {
            this.treadEffectTime += Time.delta;
            if (this.treadEffectTime >= 6.0F && this.type.treadRects.length > 0) {
                Rect treadRect = this.type.treadRects[0];
                range = -(treadRect.x + treadRect.width / 2.0F) / 4.0F;
                cx = -(treadRect.y + treadRect.height / 2.0F) / 4.0F;
                ...
                this.treadEffectTime = 0.0F;
            }
        }

        int r = Math.max(Math.round(this.hitSize * 0.6F / 8.0F), 1);
        solids = 0;
        total = (r * 2 + 1) * (r * 2 + 1);
        ...
        for(int dx = -r; dx <= r; ++dx) {
            for(dy = -r; dy <= r; ++dy) {
                ...
                if (this.type.crushDamage > 0.0F && (this.walked || this.deltaLen() >= 0.01F) && t != null && t.build != null && t.build.team != this.team && Math.max(Math.abs(dx), Math.abs(dy)) <= r - 1) {
                    t.build.damage(this.team, this.type.crushDamage * Time.delta * t.block().crushDamageMultiplier * Vars.state.rules.unitDamage(this.team));
                }
            }
        }
    ...
    public void kill() {
        if ((new Random()).nextInt(10) + 1 <= againLevel) {
            FTankUnit fu = (FTankUnit) type.create(team);
            fu.x(x);
            fu.y(y);
